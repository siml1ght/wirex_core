...
[[ -n "$SECRET_KEY" && -n "$HOP_BASE" && -n "$HOP_RANGE" ]] || {
...
echo "hydra://${PUBLIC_IP}:53?secret_key=${SECRET_KEY}&session_id=<unique-per-user>&hop_base=${HOP_BASE}&hop_range=${HOP_RANGE}&relay_port=16335"
echo
