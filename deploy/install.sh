...
EXEC_ARGS="--secret ${SECRET_KEY} --hop-base ${HOP_BASE} --hop-range ${HOP_RANGE}"
...
sleep 1
systemctl is-active --quiet "${SERVICE_NAME}" || die "service failed to start: journalctl -u ${SERVICE_NAME} -e"
...
SHARE_LINK="hydra://${PUBLIC_IP}:53?secret_key=${SECRET_KEY}&session_id=${SESSION_ID}&hop_base=${HOP_BASE}&hop_range=${HOP_RANGE}&relay_port=${RELAY_PORT}"
